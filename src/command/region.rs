...
use super::commands::move_cursor::ForwardChar;
use super::commands::move_cursor::NextLine;
use super::commands::move_cursor::PreviousLine;
...
pub fn get_region(editor: &mut Editor, jump_command_data: JumpCommandData) -> Region {
    if is_line_oriented_command(jump_command_data) {
        ...
        get_region_from_end_of_line_command(editor, jump_command_data)
    } else {
        get_region_from_command(editor, jump_command_data)
    }
}

fn get_region_from_line_oriented_command(editor: &mut Editor, jump_command_data: JumpCommandData) -> Region {
...
fn get_region_from_end_of_line_command(editor: &mut Editor, jump_command_data: JumpCommandData) -> Region {
    let start_cursor_data = editor.snapshot_cursor_data();
    let mut next_line = NextLine;
    next_line.execute(editor);
    let mut move_beginning_of_line = MoveBeginningOfLine;
...
fn get_region_from_command(editor: &mut Editor, jump_command_data: JumpCommandData) -> Region {
    let start_cursor_data = editor.snapshot_cursor_data();
    let command_data: CommandData = jump_command_data.into();
    for _ in 0..command_data.count {
        let mut jump_command = command_factory(&command_data);
