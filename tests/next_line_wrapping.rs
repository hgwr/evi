...
fn next_line_moves_to_next_buffer_line_once() {
    ...
    assert_eq!(ed.cursor.row, 1, "NextLine should move to following buffer line");
}
...
fn visual_row_progress_can_be_simulated() {
    ...
    ed.cursor.row = 0;
    ed.cursor.col = 11; // into second visual segment
    // approximate screen row as 1 (second visual row)
    ed.window_top_row = 0; // (screen position recomputed dynamically)
    let mut next = NextLine;
    next.execute(&mut ed).unwrap();
...
fn next_line_at_last_line_only_scrolls_when_possible() {
    ...
    ed.cursor.row = 1; // move to last buffer line
    let mut next = NextLine;
    // executing NextLine at last line should not move row further
    let original_row = ed.cursor.row;
    next.execute(&mut ed).unwrap();
    assert_eq!(ed.cursor.row, original_row);
}
